...
struct JSONRetriever {
    ...
    static func getJson(url: URL, httpMethod: String, queryItems: [URLQueryItem]? = nil) async throws -> [String: Any] {
        do {
    ...
    static func getJson<T: Encodable>(url: URL, httpMethod: String,  httpBodyToEncode: T, queryItems: [URLQueryItem]? = nil) async throws -> [String: Any] {
        do {
            var req = URLRequest(url: url, timeoutInterval: 60)
            req.httpMethod = httpMethod
    ...
    static func getJson2<T: Encodable>(url: URL, httpMethod: String,  httpBodyToEncode: T, queryItems: [URLQueryItem]? = nil) async throws -> Data {
        do {
            var req = URLRequest(url: url, timeoutInterval: 60)
            req.httpMethod = httpMethod
            req.setValue("application/json", forHTTPHeaderField: "Content-Type")
